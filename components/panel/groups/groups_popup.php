...
if($type=='edit'){
    ...
    $sql = "SELECT * FROM team_groups WHERE group_id=$id";
...
?>
<form action="scripts/groups/<?=$type?>_group.php" method="POST" class="text-white flex flex-col h-full gap-4 px-2 pb-8" enctype="multipart/form-data">
       ...
       <div class="grid grid-cols-8 gap-8">
        <div class="col-span-8">
          <div class="sm:grid grid-cols-4 gap-2">
            <div class="px-4 py-2 sm:px-0 col-span-4">
                <dt class="text-sm font-medium leading-6 text-gray-300">Nazwa grupy</dt>
      ...
      <div onclick="event.cancelBubble=true;" id="pupupFaqDeleteOutput">
        <div class="relative transform overflow-hidden rounded-lg bg-[#1c1c1c] px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
          ...
          <div class="sm:flex sm:items-start">
            ...
            <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
              <h3 class="text-base font-semibold leading-6 text-gray-200" id="modal-title">Usuń grupę</h3>
              <div class="mt-2">
                <p class="text-sm text-gray-400">Czy na pewno chcesz usunąć tą grupę? Nie ma możliwości przywrócenia tych danych.</p>
              </div>
            </div>
          </div>
          <form action="scripts/groups/delete_group.php?id=<?=$id?>" method="POST" class="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
